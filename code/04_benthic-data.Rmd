...
To make sure the all the code was working, I tested this out with "SCTW_2010-07-29" and "SCTW_2020-07-29". The issue comes from scenarios where species were counted at more than one transect - for example, at SCTW_2020-07-29, EC was at both transect 2 and 3, but I only want the dry/wet biomass for EC for the whole survey.  
...
ggplot(prop_biomass %>% filter(site == "MOHK")) +
  aes(x = date, y = percent_sp_dry) +
  # calculating sum of proportion within stat_summary
  stat_summary(aes(col = taxon_phylum),
               fun = "sum", geom = "line", size = 1) +
  ...
  labs(x = "Sampling date",
       y = "Proportion total biomass",
       title = "Biomass by phylum through time for MOHK",
       col = "Phylum")
...
Use `rmarkdown::render(here::here("code", "04_benthic-data.Rmd"))` to knit the document.

...
```{r urchin-plot}
urchin_plot <- ggplot(urchins %>% filter(site %in% c("aque", "napl", "ivee", "mohk", "carp")), aes(x = reorder(site, mean_urchins), y = mean_urchins)) +
...
urchins_time_plot <- urchins %>% 
  ...
  filter(year > 2015) %>% 
  ggplot(aes(x = year, y = mean_urchins)) +
...
urchin_aov <- urchins %>% 
  ...
  # filter(year > 2015) %>% 
  aov(na.omit(mean_urchins) ~ site, data = .)
...
urchin_plot_summary <- urchins %>% 
  ...
  # filter(year > 2015) %>% 
  group_by(site) %>% 
  summarize(mean = mean(mean_urchins, na.rm = TRUE),
            se = se(mean_urchins), 
            sd = sd(mean_urchins)) %>% 
  left_join(., urchin_cld_df, by = c("site" = "name")) %>% 
  mutate(value = fct_relevel(value, "a", "b", "bc", "c")) %>% 
  arrange(value) %>% 
  mutate(site = fct_relevel(site, "carp", "mohk", "napl", "aque", "ivee"))
...
urchin_plot_new <- ggplot(urchin_plot_summary, aes(x = site, y = mean)) +
  ...
  geom_text(aes(label = value, y = mean + se + 10), size = 5) + 
  labs(x = "Site", y = "Mean urchin density (number/m2)",
       title = "Purple urchin density") +
  scale_y_continuous(expand = c(0, 0),
                     limits = c(0, 200)) +
...
```{r}
community_matrix <- biomass %>% 
  filter(sp_code %in% unique(metadata_ind$sp_code)) %>% 
  filter(!(sp_code == "MAPY")) %>% 
  filter(site %in% sites_proposal_new) %>% 
...
# 647 transects, 595 with more than 0

community_metadata <- biomass %>% 
  filter(sp_code %in% unique(metadata_ind$sp_code)) %>% 
  select(sample_ID, year, month, date, site, transect) %>% 
  unique() %>% 
...
sp_rich_site_mod <- glmmTMB(richness ~ year + (1|site) + (1|transect),
                            data = sp_rich,
                            family = poisson(link = "log"))

simulateResiduals(sp_rich_site_mod) %>% plot()

summary(sp_rich_site_mod)

ggpredict(sp_rich_site_mod,
          terms = c("year")) %>% 
  plot(show_data = TRUE)
```
...
sp_div_site_mod <- glmmTMB(shandiv ~ year + (1|site) + (1|transect),
                            data = sp_div)

simulateResiduals(sp_div_site_mod) %>% plot()

summary(sp_div_site_mod)

ggpredict(sp_div_site_mod,
          terms = c("year")) %>% 
  plot(show_data = TRUE)
```
...
# write_rds(sim, here::here("data", "rds-objects", "simper.rds"))

sim <- read_rds(here::here("data", "rds-objects", "simper.rds"))
...
```{r nmds-jacc}
community_mds_jacc <- metaMDS(community_matrix, distance = "jaccard")

community_dist_jacc <- vegdist(community_matrix, method = "jaccard")
...
perm_jacc <- adonis2(community_dist_jacc ~ year, 
                     data = community_metadata,
                     strata = community_metadata$site)

perm_jacc

disper_jacc <- betadisper(community_dist_jacc, community_metadata$year)
...
fit_jacc <- community_metadata  %>% 
  dplyr::select(sample_ID, site, 
                mean_urchins, max_subs, mean_kelp, mean_umol) %>% 
...
```{r nmds-bray}
community_mds_euclidean <- metaMDS(community_matrix_transect_sub, distance = "euclidean")

community_dist_euclidean <- vegdist(community_matrix_transect_sub, method = "euclidean")

stressplot(community_mds_euclidean)

community_mds_sites_euclidean <- scores(community_mds_euclidean, display = "sites") %>% 
  as.data.frame() %>% 
  rownames_to_column("transect_ID") %>% 
  left_join(., community_metadata_transect_sub, by = "transect_ID") 

community_mds_spp_euclidean <- scores(community_mds_euclidean, display = "species") %>% 
  as.data.frame() %>% 
  rownames_to_column("sp_code") %>% 
  left_join(., coarse_traits, by = "sp_code") %>% 
  filter(sp_code %in% algae_proposal)

perm_euclidean <- adonis2(community_matrix_transect ~ site, data = community_metadata_transect, method = "euclidean")

perm_euclidean

disper_euclidean <- betadisper(community_dist_euclidean, community_metadata_transect_sub$site)

anova(disper_euclidean)

TukeyHSD(disper_euclidean)

fit_euclidean <- community_metadata_transect_sub %>% 
  dplyr::select(transect_ID, site, mean_urchins, mean_subs, mean_kelp, mean_umol) %>% 
  mutate(across(.cols = 3:4, ~replace(., is.na(.), 0))) %>% 
  envfit(community_mds_euclidean, ., perm = 999, na.rm = TRUE)

fit_vect_euclidean <- scores(fit_euclidean, display = "vectors") %>% 
  as.data.frame()

nmds_plot_euclidean <- ggplot() +
  geom_point(data = community_mds_sites_euclidean, aes(x = NMDS1, y = NMDS2, color = site, shape = site)) +
  # scale_color_manual(values = pal) +
  geom_vline(xintercept = c(0), color = "grey70", linetype = 2) +
  geom_hline(yintercept = c(0), color = "grey70", linetype = 2) +
  stat_ellipse(data = community_mds_sites_euclidean, aes(x = NMDS1, y = NMDS2, color = site), level = 0.95, size = 1) +
  geom_segment(data = community_mds_spp_euclidean, aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), arrow = arrow(length = unit(0.2, "cm"))) +
  geom_text(data = community_mds_spp_euclidean, aes(x = NMDS1, y = NMDS2, label = scientific_name)) +
  geom_segment(data = fit_vect_euclidean, aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), arrow = arrow(length = unit(0.2, "cm")), col = "blue") +
  geom_text(data = fit_vect_euclidean, aes(x = NMDS1, y = NMDS2, label = rownames(fit_vect_euclidean)), col = "blue") +
  theme_bw() +
...
# 
# ggplot(df, aes(x = max_subs, y = mean_gf_dry)) +
#   geom_point() +
#   geom_smooth(method = "lm", se = FALSE) 
# 
...
# model1 <- glmmTMB(mean_gf_dry ~ urchin_density + max_subs + urchin_density*max_subs + (1|site.x) + (1|year.x), 
...
# model2 <- glmmTMB(total_gf_dry ~ urchin_density + max_subs + (1|site.x) + (1|year.x), 
...
# 
# # GLM with beta
# model1.prop <- glmmTMB(percent_gf_dry ~ urchin_density + max_subs + mean_kelp + (1|site.x) + (1|year.x),
...
trait_matrix <- coarse_traits %>% 
  select(scientific_name, growth_morph, growth_form, pigment_type, 
...
# full model
model_lm_01a <- glmmTMB(growth_form_leathery_macrophyte ~ urchin_density + max_subs + kelp_dry + urchin_density*max_subs + (1|site) + (1|year), 
              data = CWM_cat, ziformula = ~ urchin_density + max_subs + urchin_density*max_subs, family = beta_family(link = "logit"))
...
model_lm_01b <- glmmTMB(growth_form_leathery_macrophyte ~ urchin_density + max_subs + kelp_dry + (1|site) + (1|year), 
              data = CWM_cat, ziformula = ~ urchin_density + max_subs + kelp_dry, family = beta_family(link = "logit"))
...
```{r correlations}
ggplot(CWM_cat, aes(x = urchin_density, y = max_subs)) +
  geom_point(aes(col = site)) +
  geom_smooth()

# create correlation matrix
cor_mat <- cor(CWM_cat[, c("urchin_density", "max_subs", "mean_daily_umol", "kelp_dry")], use = "na.or.complete")
...
scaled_coef <- CWM_fixed %>% 
  dplyr::select(transect_ID, mean_urchins, max_subs, mean_subs, mean_kelp, mean_umol) %>% 
...
model_thickness_01a <- glmmTMB(thickness_mm_mean_fixed ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 + (1|transect) + (1|year),
...
ggplot(CWM_fixed_filtered, aes(x = mean_urchins, y = tdmc_mean_fixed)) +
  geom_point(aes(col = site)) +
...
model_tdmc_01a <- glmmTMB(tdmc_mean_fixed ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 (1|site) + (1|year),
...
# no urchins
model_tdmc_01b <- glmmTMB(tdmc_mean_fixed ~ z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass) 

# no substrate
model_tdmc_01c <- glmmTMB(tdmc_mean_fixed ~ z.mean_urchins + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass) 

# no kelp
model_tdmc_01d <- glmmTMB(tdmc_mean_fixed ~ z.mean_urchins + z.mean_subs + z.mean_umol + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass) 

# no irradiance
model_tdmc_01e <- glmmTMB(tdmc_mean_fixed ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_urchins*z.mean_kelp +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass) 

# no sand and no kelp
model_tdmc_01f <- glmmTMB(tdmc_mean_fixed ~ z.mean_urchins + z.mean_umol + z.mean_urchins*z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass) 

# no sand and no kelp and no interaction
model_tdmc_01g <- glmmTMB(tdmc_mean_fixed ~ z.mean_urchins + z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass) 

# plot residuals
plot(simulateResiduals(model_tdmc_01a))
plot(simulateResiduals(model_tdmc_01b))
plot(simulateResiduals(model_tdmc_01c))
check_model(model_tdmc_01c)
plot(simulateResiduals(model_tdmc_01d))
plot(simulateResiduals(model_tdmc_01e))
plot(simulateResiduals(model_tdmc_01f))
plot(simulateResiduals(model_tdmc_01g))

summary(model_tdmc_01a)
summary(model_tdmc_01b)
summary(model_tdmc_01c)
summary(model_tdmc_01g)

AIC(model_tdmc_01a, model_tdmc_01b, model_tdmc_01c, model_tdmc_01d, model_tdmc_01e, model_tdmc_01f, model_tdmc_01g)

predicted_fixed <- ggpredict(model_tdmc_01g, terms = ~ z.mean_urchins + (1|site), type = "random") %>% 
  rename("site" = group)

ggplot() +
  geom_point(data = CWM_fixed_filtered, aes(x = z.mean_urchins, y = tdmc_mean_fixed, col = site, shape = site), size = 3) +
  # scale_color_gradient2(low = "green", mid = "blue", high = "yellow") +
  # new_scale("col") +
  geom_line(data = predicted_fixed, aes(x = x, y = predicted, col = site), size = 1) + 
  theme_bw() +
  labs(x = "Urchin biomass (z-score standardized)",
       y = "Community weighted mean thallus dry matter content") +
  scale_x_continuous(expand = c(0, 0)) +
...
tdmc_model_tbl <- model_tdmc_01g %>% 
  tbl_regression(intercept = TRUE) %>% 
   bold_p(t = 0.05)

MuMIn::r.squaredGLMM(model_tdmc_01g)
```

### iii. SA:P ratio
```{r sa-to-perimeter}
ggplot(CWM_fixed, aes(x = mean_urchins, y = sap_mean_fixed)) +
  geom_point(aes(col = site)) +
  geom_smooth(aes(col = site), method = "lm")
...
# full
model_sap_01a <- glmmTMB(sap_mean_fixed ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass) 

# no urchins
model_sap_01b <- glmmTMB(sap_mean_fixed ~ z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass)

# no substrate
model_sap_01c <- glmmTMB(sap_mean_fixed ~ z.mean_urchins + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass) 

# no kelp
model_sap_01d <- glmmTMB(sap_mean_fixed ~ z.mean_urchins + z.mean_subs + z.mean_umol + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass) 

# no irradiance
model_sap_01e <- glmmTMB(sap_mean_fixed ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_urchins*z.mean_kelp +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass) 

# plot residuals
plot(simulateResiduals(model_sap_01a))
plot(simulateResiduals(model_sap_01b))
plot(simulateResiduals(model_sap_01c))
plot(simulateResiduals(model_sap_01d))
plot(simulateResiduals(model_sap_01e))

summary(model_sap_01a)
summary(model_sap_01b)
summary(model_sap_01c)
summary(model_sap_01d)
summary(model_sap_01e)

AIC(model_sap_01a, model_sap_01b, model_sap_01c, model_sap_01d, model_sap_01e)

predicted_sap <- ggpredict(model_sap_01d, terms = ~ z.mean_subs + (1|site), type = "random") %>% 
  rename("site" = group)

ggplot() +
  geom_point(data = CWM_fixed_filtered, aes(x = z.mean_subs, y = sap_mean_fixed, col = site)) +
  geom_line(data = predicted_sap, aes(x = x, y = predicted, col = site), size = 1) + 
  theme_bw() +
  labs(x = "Hard substrate cover (z-score standardized)",
       y = "Community weighted mean surface area:perimeter ratio") +
  scale_x_continuous(expand = c(0, 0)) +
  # scale_y_continuous(limits = c(0, 0.45)) +
  theme(legend.position = c(0.95, 0.85))

  
sap_model_tbl <- model_sap_01b %>% 
  tbl_regression(intercept = TRUE) %>% 
   bold_p(t = 0.05)

MuMIn::r.squaredGLMM(model_sap_01b)
```
...
# full
model_fvfm_01a <- glmmTMB(fvfm_mean_fixed ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "log"), na.action = na.pass) 
...
predicted_fvfm <- ggpredict(model_fvfm_01b, terms = c("z.mean_umol", "site"), type = "random")

ggplot() +
  geom_point(data = CWM_fixed_filtered, aes(x = z.mean_umol, y = fvfm_mean_fixed, col = site)) +
...
# full
model_max_height_cm_01a <- glmmTMB(max_height_cm_fixed ~ urchin_density + mean_subs + mean_daily_umol + kelp_dry + (1|site) + (1|year),
        data = CWM_fixed, family = gaussian(link = "identity")) 

# site as fixed instead of random effect
model_max_height_cm_01b <- glmmTMB(max_height_cm_fixed ~ urchin_density + mean_subs + mean_daily_umol + kelp_dry + site + (1|year),
        data = CWM_fixed, family = gaussian(link = "identity")) 

# no site
model_max_height_cm_01c <- glmmTMB(max_height_cm_fixed ~ urchin_density + mean_subs + mean_daily_umol + kelp_dry + (1|year),
        data = CWM_fixed, family = gaussian(link = "identity")) 
...
```{r}
ggplot(CWM_fixed_filtered, aes(x = mean_umol, y = sta_mean_fixed)) +
  geom_point(aes(col = site)) +
  geom_smooth(method = "lm")
...
# full
model_sta_mean_01a <- glmmTMB(sta_mean_fixed ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass)

model_sta_mean_01b <- glmmTMB(sta_mean_fixed ~ z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_kelp*z.mean_umol + (1|site) + (1|year),
        data = CWM_fixed_filtered, family = gaussian(link = "identity"), na.action = na.pass)

# plot residuals
plot(simulateResiduals(model_sta_mean_01a))

plot(simulateResiduals(model_sta_mean_01b))

summary(model_sta_mean_01a)
summary(model_sta_mean_01b)

AIC(model_sta_mean_01a, model_sta_mean_01b)

predicted_sta <- ggpredict(model_sta_mean_01b, terms = ~ z.mean_umol + (1|site), type = "random")

ggplot() +
  geom_point(data = CWM_fixed_filtered, aes(x = z.mean_umol, y = sta_mean_fixed, col = site, shape = site), size = 3) +
  geom_line(data = predicted_sta, aes(x = x, y = predicted, col = group), size = 1) +
  # geom_ribbon(data = predicted_sta, aes(x = x, ymax = conf.high, ymin = conf.low, fill = group), alpha = 0.05) +
  labs(x = "Irradiance (z-score standardized)",
       y = "Community weighted mean of STA") +
  theme_bw() +
  theme(legend.position = c(0.95, 0.85))

sta_model_tbl <- model_sta_mean_01b %>% 
  tbl_regression(intercept = TRUE) %>% 
  bold_p(t = 0.05)
  
MuMIn::r.squaredGLMM(model_sta_mean_01a)

tbl_merge(tbls = list(tdmc_model_tbl, sta_model_tbl, sap_model_tbl),
          tab_spanner = c("TDMC", "STA", "SA:P"))
```
...
model_lm_01a <- glmmTMB(growth_form_leathery_macrophyte_fixed ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_urchins*z.mean_subs + z.mean_urchins*z.mean_kelp + z.mean_kelp*z.mean_subs + (1|site) + (1|year), 
                        ziformula = z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_urchins*z.mean_subs + z.mean_urchins*z.mean_kelp + z.mean_kelp*z.mean_subs + (1|site) + (1|year),
...
### viii. coloring NMDS results by CWM

```{r}
CWM_NMDS <- community_mds_sites_euclidean %>% 
  dplyr::select(transect_ID, NMDS1, NMDS2) %>% 
  left_join(., CWM_fixed_filtered, by = "transect_ID") %>% 
  mutate_at(4:31, as.numeric)

nmds_plot_euclidean <- ggplot() +
  geom_point(data = CWM_NMDS, aes(x = NMDS1, y = NMDS2, color = tdmc_mean_fixed, shape = site), size = 2) +
  scale_colour_gradient(low = "yellow", high = "darkblue") +
  new_scale_color() + 
  # scale_color_manual(values = pal) +
  geom_vline(xintercept = c(0), color = "grey70", linetype = 2) +
  geom_hline(yintercept = c(0), color = "grey70", linetype = 2) +
  # stat_ellipse(data = CWM_NMDS %>% filter(site != "NA"), aes(x = NMDS1, y = NMDS2, color = site), level = 0.95, size = 1) +
  # geom_segment(data = community_mds_spp_euclidean, aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), arrow = arrow(length = unit(0.2, "cm"))) +
  # geom_text(data = community_mds_spp_euclidean, aes(x = NMDS1, y = NMDS2, label = scientific_name)) +
  geom_segment(data = fit_vect_euclidean, aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), arrow = arrow(length = unit(0.2, "cm")), col = "blue") +
  geom_text(data = fit_vect_euclidean, aes(x = NMDS1, y = NMDS2, label = rownames(fit_vect_euclidean)), col = "blue") +
...
CWM_bull <- functcomp(x = bull_tbspp, a = bull_commat, 
  ...
  left_join(., CWM_fixed, by = "transect_ID")
```
...
```{r all-together}
df <- CWM_mohk %>% 
  select(transect_ID, fvfm_mean_specif, tdmc_mean_specif, sta_mean_specif, thickness_mm_mean_specif, site) %>% 
  rbind(., (CWM_ivee %>% select(transect_ID, fvfm_mean_specif, tdmc_mean_specif, sta_mean_specif, thickness_mm_mean_specif, site))) %>% 
  rbind(., (CWM_aque %>% select(transect_ID, fvfm_mean_specif, tdmc_mean_specif, sta_mean_specif, thickness_mm_mean_specif, site))) %>% 
  left_join(., CWM_fixed_filtered, by = "transect_ID") %>% 
  rename("site" = site.x)
```
...
```{r variance-decomp}
tdmc_tf <- traitflex.anova(~ site, specif.avg = tdmc_mean_specif, const.avg = tdmc_mean_fixed, data = df)
tdmc_tf
plot(tdmc_tf)

thickness_tf <- traitflex.anova(~ site, specif.avg = thickness_mm_mean_specif, const.avg = thickness_mm_mean_fixed, data = df)
thickness_tf
plot(thickness_tf)

sta_tf <- traitflex.anova(~ site, specif.avg = sta_mean_specif, const.avg = sta_mean_fixed, data = df)
sta_tf
plot(sta_tf)
```
...
### i. continuous and categorical traits

```{r}
tbspp_mat_num <- ind_traits %>% 
  filter(sp_code %in% algae_proposal) %>% 
  ...
  dplyr::select(specimen_ID, maximum_height, sta_mean, tdmc_mean, sav_mean, sap_mean, fvfm_mean, total_volume, total_dry, thickness_mm_mean, frond_length_mean, frond_width_mean, growth_form_num, pigment_type_num, longevity_num) %>% 
  left_join(., metadata_ind, by = "specimen_ID") %>% 
  group_by(sp_code) %>% 
  summarize(across(2:15, ~ mean(.x, na.rm = TRUE))) %>% 
  column_to_rownames("sp_code")

commat_dbFD <- biomass %>% 
  dplyr::select(sample_ID, sp_code, dry_gm2) %>% 
  pivot_wider(names_from = sp_code, values_from = dry_gm2) %>% 
    # filter(sample_ID %in% CWM_fixed_filtered$transect_ID) %>% 
  column_to_rownames("sample_ID") %>% 
  mutate_all(~replace(., is.na(.), 0)) %>% 
  dplyr::select(c(rownames(tbspp_mat_num))) %>% 
  # take out 0 sites
  mutate(total = rowSums(across(where(is.numeric)))) %>% 
  filter(total > 0) %>% 
  dplyr::select(-total) %>% 
...
fd_cont <- dbFD(x = tbspp_mat_num, a = commat_dbFD)
...
# FRic: Quality of the reduced-space representation = 0.738934 
# FDiv: Could not be calculated for communities with <3 functionally singular species. 

fd_cont

# species richness
spp_rich <- specnumber(commat_dbFD) %>% 
  as.data.frame() %>% 
  rename("spprich" = '.') %>% 
  rownames_to_column("sample_ID")

spp_div <- diversity(commat_dbFD, index = "shannon") %>% 
  as.data.frame() %>% 
  rename("shandiv" = '.') %>% 
  rownames_to_column("sample_ID")

fric_df <- fd_cont$FRic %>% 
  as.data.frame() %>% 
  rename("fric" = '.') %>% 
  rownames_to_column("sample_ID")

fdis_df <- fd_cont$FDis %>% 
  as.data.frame() %>% 
  rename("fdis" = '.') %>% 
  rownames_to_column("sample_ID")

fdiv_df <- fd_cont$FDiv %>% 
  as.data.frame() %>% 
  rename("fdiv" = '.') %>% 
  rownames_to_column("sample_ID")

fd_sum_df <- fd_cont$FEve %>% 
  as.data.frame() %>% 
  rename("feve" = '.') %>% 
  rownames_to_column("sample_ID") %>% 
  left_join(., fric_df, by = "sample_ID") %>% 
  left_join(., fdiv_df, by = "sample_ID") %>% 
  left_join(., fdis_df, by = "sample_ID") %>% 
  left_join(., community_metadata, by = "sample_ID") %>% 
  left_join(., spp_rich, by = "sample_ID") %>% 
  left_join(., spp_div, by = "sample_ID") %>% 
  drop_na()
...
mod <- glmmTMB(fric ~ spprich + (1|site/transect),
               data = fd_sum_df)
...
# richness
fric_lmm <- glmmTMB(fric ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 + (1|transect) + (1|year),
        data = fd_sum_df, family = gaussian(link = "log"), na.action = na.pass) 
plot(simulateResiduals(fric_lmm))
summary(fric_lmm)
...
ggplot(fd_sum_df) +
  geom_point(aes(x = z.mean_kelp, y = fric, col = site)) +
  geom_smooth(aes(x = z.mean_kelp, y = fric), method = "lm")

ggplot(fd_sum_df, aes(x = year, y = fric, col = site, fill = site)) +
  geom_point() +
  geom_smooth(method = "lm")

# evenness
feve_lmm <- glmmTMB(feve ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 + (1|transect) + (1|year),
        data = fd_sum_df, family = gaussian(link = "log"), na.action = na.pass) 
plot(simulateResiduals(feve_lmm))
summary(feve_lmm)

ggplot(fd_sum_df) +
  geom_point(aes(x = z.mean_kelp, y = feve, col = z.mean_umol)) +
  geom_smooth(aes(x = z.mean_kelp, y = feve), method = "lm")

ggplot(fd_sum_df) +
  geom_point(aes(x = z.mean_umol, y = feve, col = site)) +
  geom_smooth(aes(x = z.mean_umol, y = feve), method = "lm")

# divergence
fdiv_lmm <- glmmTMB(fdiv ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 + (1|site) + (1|year),
        data = fd_sum_df, family = gaussian(link = "identity"), na.action = na.pass) 
plot(simulateResiduals(fdiv_lmm))
summary(fdiv_lmm)
fdiv_lmm %>% 
  tbl_regression() %>% 
  bold_p(t = 0.05)
MuMIn::r.squaredGLMM(fdiv_lmm)

ggplot(fd_sum_df) +
  geom_point(aes(x = z.mean_kelp, y = fdiv, col = site)) +
  geom_smooth(aes(x = z.mean_kelp, y = fdiv), method = "lm") +
  theme_bw() +
  labs(x = "Kelp biomass (z-score standardized)",
       y = "Functional divergence") +
  theme(axis.title = element_text(size = 10))


# dispersion
fdis_lmm <- glmmTMB(fdis ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 + (1|site) + (1|year),
        data = fd_sum_df, family = gaussian(link = "log"), na.action = na.pass) 
plot(simulateResiduals(fdis_lmm))
summary(fdis_lmm)

ggplot(fd_sum_df) +
  geom_point(aes(x = z.mean_kelp, y = fdis, col = site)) +
  geom_smooth(aes(x = z.mean_kelp, y = fdis), method = "lm") 

fd_sum_df %>% 
  ggplot(aes(x = shandiv, y = fric)) +
  geom_point(aes(col = site)) +
  geom_smooth(method = "lm")

div_lm <- lm(fric ~ shandiv, data = fd_sum_df)
summary(div_lm)
cor.test(fd_sum_df$fric, fd_sum_df$shandiv, method = "pearson")
```

### ii. categorical traits on whole data set

```{r}
tbspp_mat_cat <- coarse_traits %>%
  dplyr::select(sp_code, growth_form, pigment_type, life_habit, longevity, posture) %>% 
  filter(sp_code != "NEO") %>% 
  column_to_rownames("sp_code") %>% 
  mutate_all(as.factor)

commat_dbFD_cat <- biomass %>% 
  ...
  pivot_wider(names_from = sp_code, values_from = dry_gm2) %>% 
  filter(transect_ID %in% CWM_fixed_filtered$transect_ID) %>% 
  column_to_rownames("transect_ID") %>% 
  mutate_all(~replace(., is.na(.), 0)) %>% 
  dplyr::select(c(rownames(tbspp_mat_cat))) %>% 
  # take out 0 sites
  mutate(total = rowSums(across(where(is.numeric)))) %>% 
  filter(total > 0) %>% 
  dplyr::select(-total) %>% 
...
# species richness
spp_rich <- specnumber(commat_dbFD_cat) %>% 
  as.data.frame() %>% 
  rename("spprich" = '.') %>% 
  rownames_to_column("transect_ID")

spp_div <- diversity(commat_dbFD_cat, index = "shannon") %>% 
  as.data.frame() %>% 
  rename("shandiv" = '.') %>% 
  rownames_to_column("transect_ID")

cat_fd <- dbFD(x = tbspp_mat_cat, a = commat_dbFD_cat, corr = c("none"))
...
fd_cat_df <- cat_fd$FEve %>% 
  as.data.frame() %>% 
  rename("feve" = '.') %>% 
  rownames_to_column("transect_ID") %>% 
  left_join(., fric_df, by = "transect_ID") %>% 
  left_join(., raoq_df, by = "transect_ID") %>% 
  left_join(., fdis_df, by = "transect_ID") %>% 
  left_join(., CWM_fixed_filtered, by = "transect_ID") %>% 
  left_join(., spp_rich, by = "transect_ID") %>% 
  left_join(., spp_div, by = "transect_ID") %>% 
  drop_na(feve)

# richness
fric_lmm <- glmmTMB(fric ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 + (1|site) + (1|year),
        data = fd_cat_df, family = gaussian(link = "identity"), na.action = na.pass) 
plot(simulateResiduals(fric_lmm))
summary(fric_lmm)
...
feve_lmm <- glmmTMB(feve ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 + (1|site) + (1|year),
        data = fd_cat_df, family = gaussian(link = "log"), na.action = na.pass) 
plot(simulateResiduals(feve_lmm))
summary(feve_lmm)

ggplot(fd_sum_df) +
  geom_point(aes(x = z.mean_kelp, y = feve, col = site)) +
  geom_smooth(aes(x = z.mean_kelp, y = feve), method = "lm")

# divergence
raoq_lmm <- glmmTMB(raoq ~ z.mean_urchins + z.mean_subs + z.mean_kelp + z.mean_umol + z.mean_urchins*z.mean_kelp + z.mean_urchins*z.mean_umol + z.mean_subs*z.mean_umol + z.mean_kelp*z.mean_umol +
                                 + (1|site) + (1|year),
        data = fd_cat_df, family = gaussian(link = "log"), na.action = na.pass) 
plot(simulateResiduals(raoq_lmm))
summary(raoq_lmm)
raoq_lmm %>% 
  tbl_regression() %>% 
  bold_p(t = 0.05)
MuMIn::r.squaredGLMM(raoq_lmm)

ggplot(fd_cat_df) +
  geom_point(aes(x = z.mean_kelp, y = raoq, col = site)) +
  geom_smooth(aes(x = z.mean_kelp, y = raoq), method = "lm", se = FALSE) +
...
ggplot(fd_cat_df) +
  geom_jitter(aes(x = z.mean_kelp, y = spprich, col = site)) +
  geom_smooth(aes(x = z.mean_kelp, y = spprich), method = "lm")

ggplot(fd_cat_df) +
  geom_jitter(aes(x = z.mean_kelp, y = fric, col = site)) +
  geom_smooth(aes(x = z.mean_kelp, y = fric), method = "lm")

fd_cat_df %>% 
  ggplot(aes(x = spprich, y = fric)) +
  geom_point(aes(col = site)) +
  geom_smooth()

rich_lm <- lm(raoq ~ shandiv, data = fd_cat_df)
summary(rich_lm)

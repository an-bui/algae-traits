...
perm_bray <- adonis2(community_dist_bray ~ site, data = community_metadata)

perm_bray

disper_bray <- betadisper(community_dist_bray, community_metadata$site)

anova(disper_bray)

TukeyHSD(disper_bray)

fit_bray <- community_metadata %>% 
  select(sample_ID, site, urchin_density, mean_sand, mean_daily_umol, kelp_dry) %>% 
  mutate(across(.cols = 3:4, ~replace(., is.na(.), 0))) %>% 
...
```{r prop-test, include = FALSE, eval = FALSE}
...
temp <- ggpredict(model_lm_01a, terms = c("urchin_density", "site"), type = "re")

# predict(model_lm_01a)
...
model_thickness_01a <- glmmTMB(thickness_mm_fixed ~ urchin_density + mean_sand + kelp_dry + (1|site) + (1|year),
        data = CWM_fixed, family = Gamma(link = "logit")) 

model_thickness_01b <- glmmTMB(thickness_mm_fixed ~ urchin_density + mean_sand + kelp_dry + (1|year),
...
### ii. LDMC/TDMC
```{r ldmc}
ggplot(CWM_fixed, aes(x = site, y = tdmc_fixed)) +
  geom_boxplot() +
  geom_jitter(alpha = 0.5)

tdmc_aov <- aov(tdmc_fixed ~ site, data = CWM_fixed)
summary(tdmc_aov)
TukeyHSD(tdmc_aov)
...
model_tdmc_01 <- glmmTMB(tdmc_fixed ~ urchin_density + mean_sand + kelp_dry + (1|site) + (1|year),
        data = CWM_fixed, family = Gamma(link = "logit")) 

testDispersion(model_tdmc_01)
testResiduals(model_tdmc_01)

model_tdmc_02 <- lmer(tdmc_fixed ~ urchin_density + mean_sand + kelp_dry + (1|site) + (1|year),
        data = CWM_fixed)

model_tdmc_03 <- lmer(tdmc_fixed ~ urchin_density + mean_sand + kelp_dry + urchin_density*mean_sand + (1|site) + (1|year),
        data = CWM_fixed)

model_tdmc_04 <- glmmTMB(tdmc_fixed ~ urchin_density + mean_sand + kelp_dry + urchin_density*mean_sand + (1|site) + (1|year),
        data = CWM_fixed, family = gaussian(link = "identity"))
...
model_tdmc_03_resid <- simulateResiduals(model_tdmc_03)
plot(model_tdmc_03_resid)

model_tdmc_04_resid <- simulateResiduals(model_tdmc_04)
plot(model_tdmc_04_resid)

summary(model_tdmc_01)
summary(model_tdmc_02)
summary(model_tdmc_03)
summary(model_tdmc_04)

AIC(model_tdmc_01, model_tdmc_02, model_tdmc_03, model_tdmc_04)
```
...
# full
model_ratio_01 <- glmmTMB(ratio_fixed ~ urchin_density + mean_sand + kelp_dry + urchin_density*mean_sand + (1|site) + (1|year),
        data = CWM_fixed, family = Gamma(link = "logit")) 

model_ratio_02 <- glmmTMB(ratio_fixed ~ urchin_density + mean_sand + kelp_dry + (1|site) + (1|year),
        data = CWM_fixed, family = Gamma(link = "logit")) 
...
### iv. FvFm
```{r fvfm}
ggplot(CWM_fixed, aes(x = site, y = fvfm_fixed)) +
  geom_boxplot() +
  geom_jitter(alpha = 0.5)
...
# full
model_fvfm_01a <- glmmTMB(fvfm_fixed ~ urchin_density + mean_sand + kelp_dry + urchin_density*mean_sand + (1|site) + (1|year),
        data = CWM_fixed, family = Gamma(link = "logit")) 

# best model
model_fvfm_01b <- glmmTMB(fvfm_fixed ~ urchin_density + mean_sand + kelp_dry + (1|site) + (1|year),
        data = CWM_fixed, family = Gamma(link = "logit")) 

# no site
model_fvfm_01c <- glmmTMB(fvfm_fixed ~ urchin_density + mean_sand + (1|site) + (1|year),
        data = CWM_fixed, family = Gamma(link = "logit")) 

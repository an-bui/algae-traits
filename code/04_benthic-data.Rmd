...
urchin_plot_summary <- urchins %>% 
  ...
  arrange(value) %>% 
  mutate(site = fct_relevel(site, "ivee", "mohk", "aque", "napl", "carp"))
...
urchin_plot_new <- ggplot(urchin_plot_summary, aes(x = site, y = mean)) +
  ...
  geom_text(aes(label = value, y = mean + se + .5), size = 5) + 
  labs(x = "Site", y = "Mean urchin density (number/m2)",
       title = "Purple urchin density") +
  scale_y_continuous(expand = c(0, 0)) +
...
perm_jacc <- adonis2(community_dist_jacc ~ site, data = community_metadata_transect_sub)

perm_jacc

disper_jacc <- betadisper(community_dist_jacc, community_metadata_transect_sub$site)

anova(disper_jacc)

TukeyHSD(disper_jacc)

fit_jacc <- community_metadata_transect_sub %>% 
  dplyr::select(sample_ID, site, mean_urchins, max_subs, mean_kelp, mean_umol) %>% 
...
fd_sum_df <- fd_cont$FEve %>% 
  ...
  left_join(., fdiv_df, by = "transect_ID") %>% 

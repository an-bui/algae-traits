...
pca_mat <- ind_traits %>% 
  ...
  # weird Nienburgia?
  filter(!(specimen_ID == "20210721-BULL-023")) %>% 
  select(specimen_ID, maximum_height, sta_mean, tdmc_mean, sav_mean, sap_mean, fvfm_mean, total_volume, total_dry, thickness_mm_mean, frond_length_mean, frond_width_mean) %>% 

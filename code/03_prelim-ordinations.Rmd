...
pca_mat <- ind_traits %>% 
  ...
  dplyr::select(specimen_ID, maximum_height, sta_mean, tdmc_mean, sav_mean, sap_mean, fvfm_mean, thickness_mm_mean, frond_length_mean, frond_width_mean) %>% 
  column_to_rownames("specimen_ID") %>% 
  drop_na()

# , growth_form_num, pigment_type_num, longevity_num total_volume, total_dry, 
```

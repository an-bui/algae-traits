...
pca_mat <- ind_traits %>% 
  ...
  dplyr::select(specimen_ID, maximum_height, total_volume, sav_mean, thickness_mm_mean, tdmc_mean, sav_mean, sap_mean, fvfm_mean, thickness_mm_mean) %>% 
  column_to_rownames("specimen_ID") %>% 
  drop_na()
...
thick_sav <- ggplot(tradeoff_summ,
  ...
  scale_color_manual(values = algae_colors) +
  annotate(geom = "text", x = 1, y = 7500, label = "Pearson's r = -0.645") +
...
sap_sav <- ggplot(tradeoff_summ,
  ...
  scale_fill_manual(values = algae_colors) +
  scale_color_manual(values = algae_colors) +
  annotate(geom = "text", x = 60, y = 7500, label = "Pearson's r = -0.389") +
...
sap_vol <- ggplot(tradeoff_summ,
  ...
  scale_fill_manual(values = algae_colors) +
  scale_color_manual(values = algae_colors) +
  annotate(geom = "text", x = 60, y = 1000, label = "Pearson's r = 0.409") +
  labs(x = "Mean surface area:perimeter ratio (mm)",
       y = "Mean volume (mL)") +
  theme_bw() +
  theme(legend.position = "none",
...
tdmc_fvfm <- ggplot(tradeoff_summ,
  ...
  scale_fill_manual(values = algae_colors) +
  scale_color_manual(values = algae_colors) +
  annotate(geom = "text", x = 0.75, y = 0.8, label = "Pearson's r = -0.446") +

...
pca_mat_log <- pca_mat %>% 
  mutate(across(c(`Maximum height`, `Mass:height`, `SA:V`,
                `Thickness`, `STA`, `SA:P`, `Aspect ratio`), log))
...
loadings_plot_aes <- list(
  ...
  scale_x_continuous(limits = c(-0.89, 1), breaks = seq(from = -0.75, to = 1, by = 0.25))
)
...
plot_PCA_12 <- ggplot() +
  ...
  geom_text_repel(data = PCAvect, aes(x = PC1, y = PC2, label = rownames(PCAvect)), size = 8, alpha = 0.8,
            color = "red") +
...
plot_PCA_13 <- ggplot() +
  ...
  geom_text_repel(data = PCAvect_13, aes(x = PC1, y = PC3, label = rownames(PCAvect)), size = 8, alpha = 0.8,
            color = "red") +
...
```{r}
pairs_plot <- GGally::ggpairs(pca_mat_log, 
...
sta_sav <- lmodel2(scale(sta_mean) ~ scale(sav_mean),
                   data = ind_traits_filtered)

sma(scale(sta_mean) ~ scale(sav_mean),
    data = ind_traits_filtered)
...
sta_sav <- ggplot(data = ind_traits_filtered,
       aes(x = scale(sav_mean),
           y = scale(sta_mean))) +
  geom_point() +
  stat_ma_line(method = "SMA") +
  labs(x = "scale surface area:volume ratio (mm\U00B2/mL)",
       y = "scale specific thallus area (mm\U00B2/g)") +
  theme_bw() +
  theme(panel.grid = element_blank()) +
  coord_cartesian(ylim = c(0, 16)) +
  annotate(geom = "text",
           x = 4, y = 15,
           label = "R\U00B2 = 0.05, p < 0.001")
...
mh_sta <- ggplot(data = ind_traits_filtered,
  ...
  geom_point() +
  stat_ma_line(method = "SMA") +
  labs(x = "scaled log specific thallus area (mm\U00B2/g)",
       y = "scaled mass:height ratio (g/cm)") +
  theme_bw() +
  theme(panel.grid = element_blank()) +
  coord_cartesian(ylim = c(-0.5, 7.6)) +
  annotate(geom = "text",
           x = 12, y = 7,
           label = "R\U00B2 = 0.02, p = 0.01")

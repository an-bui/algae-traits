...
pca_mat <- ind_traits %>% 
  ...
  dplyr::select(specimen_ID, sta_mean, tdmc_mean, sav_mean, sap_mean, fvfm_mean, total_volume, total_dry, thickness_mm_mean, frond_length_mean, frond_width_mean) %>% 
  column_to_rownames("specimen_ID") %>% 
  drop_na()
...
tbs_pca <- rda(pca_mat[, 1:10], scale = TRUE)

...
fvfm_df <- fvfm_ind %>% 
  ...
  left_join(., coarse_traits, by = "sp_code") %>% 
  filter(sp_code %in% algae_proposal) %>% 
  # remove GR and CO because only 1 observation
  filter(!(sp_code %in% c("GR", "CO")))
...
bartlett.test(fvfm_mean ~ site, data = fvfm_ptca)
...
fvfm_boxplot_ptca_recruit <- fvfm_ptca_recruit %>% 
  ...
  geom_jitter() +
  theme_bw() +
  labs(caption = "significantly different across sites, p < 0.001")
...
t.test(fvfm_mean ~ site, data = fvfm_ptca_adult, var.equal = TRUE)
...
fvfm_boxplot_ptca_adult <- fvfm_ptca_adult %>% 
  ...
  geom_jitter() +
  theme_bw() +
  labs(caption = "t-test: t(15) = -0.08, p = 0.94")
...
# analysis of variance
tdmc_sp_aov <- aov(tdmc ~ sp_code, data = weight_summary %>% filter(!(sp_code %in% c("BO", "CO"))) %>% filter(sp_code %in% algae_proposal))
...
## e. boxplot PTCA adults vs recruits
```{r}
tdmc_ptca <- weight_summary %>% 
  filter(sp_code == "PTCA") %>% 
  mutate(lifestage = case_when(
    date_collected %in% c("20220405", "20220407") ~ "adult",
    TRUE ~ "new_recruit"
  )) %>% 
  # take out weird sample
  filter(specimen_ID != "20210721-BULL-005-B")

var.test(tdmc ~ lifestage, data = tdmc_ptca)
# not significantly different variances
t.test(tdmc ~ lifestage, data = tdmc_ptca, var.equal = TRUE)
# signficantly different

tdmc_boxplot_ptca <- tdmc_ptca %>% 
  ggplot(aes(x = lifestage, y = tdmc)) +
  geom_boxplot(aes(fill = lifestage)) +
  ...
  labs(title = "PTCA TDMC",
       x = "lifestage",
       y = "TDMC",
       caption = "t-test: t(178) = 7.097, p < 0.001") +
  theme(legend.position = "none") +
  facet_wrap(~site)
...
```{r}
sta_ptca <- sta_summary %>% 
  filter(sp_code == "PTCA") %>% 
  filter(site %in% c("Mohawk", "Arroyo Quemado")) %>% 
  mutate(lifestage = case_when(
    date_collected %in% c("20220405", "20220407") ~ "adult",
    TRUE ~ "new_recruit"
  )) 

var.test(sta_mm_mg ~ lifestage, data = sta_ptca)
bartlett.test(sta_mm_mg ~ lifestage, data = sta_ptca)
leveneTest(sta_mm_mg ~ lifestage, data = sta_ptca)
# significantly different variances

summary(aov(sta_mm_mg ~ lifestage, 
            data = sta_ptca %>% 
              filter(site %in% c("Mohawk", "Arroyo Quemado"))))
# significantly different between lifestages
...
sta_boxplot_ptca <- sta_ptca %>% 
  filter(site %in% c("Mohawk", "Arroyo Quemado")) %>% 
  ggplot(aes(x = lifestage, y = sta_mm_mg)) +
  geom_boxplot(aes(fill = lifestage)) +
  scale_fill_manual(values = c("#70601d", "#b59b33")) +
  geom_jitter() +
  theme_bw() +
...
```{r}
ggplot(sta_summary %>% filter(!(sp_code %in% c("Nandersoniana", "CO"))) %>% drop_na(tdmc), aes(x = sta_mm_mg, y = tdmc, col = sp_code)) +
  geom_point() +
...
sta_tdmc_lmer <- lmer(tdmc ~ sta_mm_mg + (1|sp_code), data = sta_summary %>% filter(!(sp_code %in% c("Nandersoniana", "CO"))) %>% drop_na(tdmc))
sta_tdmc_lme <- lme(tdmc ~ sta_mm_mg, random = ~ 1|sp_code, data = sta_summary %>% filter(!(sp_code %in% c("Nandersoniana", "CO"))) %>% drop_na(tdmc))
summary(sta_tdmc_lme)
summary(sta_tdmc_lmer)
testResiduals(sta_tdmc_lme)
check_model(sta_tdmc_lmer)

sta_tdmc_lm <- lm(tdmc ~ sta_mm_mg, data = sta_summary %>% filter(!(sp_code %in% c("Nandersoniana", "CO"))))
summary(sta_tdmc_lm)
...
```{r}
fvfm_tdmc_lme <- lme(fvfm ~ tdmc, random = ~ 1|sp_code, data = sp_int_matrix %>% filter(!(sp_code %in% c("Nandersoniana", "CO"))) %>% drop_na(tdmc))
fvfm_tdmc_lmer <- lmer(fvfm ~ tdmc + (1|sp_code), data = sp_int_matrix %>% filter(!(sp_code %in% c("Nandersoniana", "CO"))) %>% drop_na(tdmc))
summary(fvfm_tdmc_lmer)
testResiduals(fvfm_tdmc_lmer)
check_model(fvfm_tdmc_lmer)
grid <- ref_grid(fvfm_tdmc_lmer, cov.keep = c("tdmc", "sp_code"))
emm <- emmeans(grid, spec = c("tdmc"), level = 0.95) %>% 
  data.frame(.)
...
ggplot() +
  geom_point(data = sp_int_matrix %>% filter(!(sp_code %in% c("Nandersoniana", "CO"))) %>% drop_na(tdmc), aes(x = tdmc, y = fvfm, col = sp_code, shape = sp_code), size = 2) +
  geom_smooth(data = sp_int_matrix %>% filter(!(sp_code %in% c("Nandersoniana", "CO"))) %>% drop_na(tdmc), aes(x = tdmc, y = fvfm, col = sp_code), method = "lm", se = FALSE) +
  geom_line(data = emm, aes(x = tdmc, y = emmean)) +
  geom_ribbon(data = emm, aes(x = tdmc, y = emmean, ymin = lower.CL, ymax = upper.CL), alpha = 0.2) +

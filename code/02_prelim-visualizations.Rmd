...
fvfm_boxplot_ptca_adult <- fvfm_ptca_adult %>% 
  ...
  geom_jitter() +
  theme_bw() +
  labs(caption = "t-test: t(16) = -0.208, p = 0.84")
...
bartlett.test(thickness_mm_mean ~ lifestage, data = ptca_thickness)
leveneTest(thickness_mm_mean ~ lifestage, data = ptca_thickness)
# not significantly different variances
...
weight_df <- weight_ind %>% 
  left_join(., metadata_ind, by = "specimen_ID") %>% 
  filter(sp_code %in% algae_proposal) %>% 
  # take out calcifying species, which do not have decalcified weight
...
# analysis of variance
tdmc_sp_aov <- aov(tdmc_mean ~ sp_code, data = ind_traits %>% filter(sp_code %in% algae_proposal) %>% filter(sp_code != "CO"))
tdmc_tax_aov <- aov(tdmc_mean ~ pigment_type, data = ind_traits %>% filter(sp_code %in% algae_proposal) %>% filter(sp_code != "CO"))
...
tdmc_boxplot <- ind_traits %>% 
  ...
  geom_boxplot(aes(fill = value)) +
  geom_jitter() +
  geom_text(aes(label = value, y = 0.275), nudge_y = 0.01) + 
  facet_wrap(~ pigment_type, scales = "free_x") +
...
```{r}
ptca_tdmc <- ind_traits %>% 
  left_join(., metadata_ind, by = "specimen_ID") %>% 
  rename("sp_code" = sp_code.x) %>% 
  rename("lifestage" = lifestage.x) %>% 
  filter(sp_code == "PTCA")

var.test(tdmc_mean ~ lifestage, data = ptca_tdmc)
# not significantly different variances
...
# test homogeneity of variance
bartlett.test(sta_mean ~ sp_code, data = sta_df)
# heterogeneous variances

# analysis of variance
sta_sp_aov <- aov(sta_mean ~ sp_code, data = sta_df)
...
sta_boxplot <- sta_df %>% 
  ...
  ggplot(aes(x = sp_code, y = sta_mean)) +
  geom_boxplot(aes(fill = value)) +
  geom_jitter() +
  geom_text(aes(label = value, y = 51), nudge_y = 0.01) + 
  facet_wrap(~ pigment_type, scales = "free_x") +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  theme_bw() +
...
var.test(sta_mean ~ site, data = ptca_sta_adults)
# not significantly different variances

summary(aov(sta_mean ~ site, 
            data = ptca_sta_adults))
...
sta_tdmc_lmer <- lmer(tdmc_mean ~ sta_mean + (1|sp_code), data = (ind_traits %>% filter(!(sp_code %in% c("CO"))) %>% filter(sp_code %in% algae_proposal) %>% drop_na(tdmc_mean, sta_mean)))

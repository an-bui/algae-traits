# This is a script for determining the sources of variation in trait values
# follows Messier et al. 2010 and Messier blog post with code: https://juliemessier.org/code/
...
# using raw data to preserve individual measurements in a thallus
...
# nested structure: species by site
fvfm_m1_lme <- lme(
  log10(fvfm_mean) ~ 1, 
  random = ~1|site/sp_code,
  data = ind_traits,
  na.action = na.omit
)

# testing with lmer
fvfm_m1_lmer <- lmerTest::lmer(
  log10(fvfm_mean) ~ 1 + (1|site/sp_code),
  data = ind_traits, 
  na.action = na.omit
)
...
# exactly the same
summary(fvfm_m1_lme)
summary(fvfm_m1_lmer)

# variance components are also the same
plot(varcomp(fvfm_m1_lme, scale = TRUE))
# 89% from species, 4.13% from site, 6.85% from individuals
plot_variance(fvfm_m1_lmer)
icc_specs(fvfm_m1_lmer)

# ⊣ ⊣ year + site + species/ind/sub --------------------------

fvfm_sub_lmer <- lmerTest::lmer(
  log10(fvfm_meas) ~ 1 + (1|year) + (1|site) + (1|sp_code/specimen_ID), 
  data = fvfm_df, 
  na.action = na.omit
)

var_decomp(fvfm_sub_lmer)
# 70.1% variation from species, 20.3% within individual, 7.1% across individuals
...
plot(varcomp(fvfm_sub_lmer, scale = TRUE))

...
keep_4trait_full_combination <- combo_4traits_full %>% 
  mutate(combo_number = rownames(.)) %>% 
  # count TRUE and FALSE occurrences
  mutate(count = map(
    pairwise_significant_euc,
...
keep_3trait_full_combination <- combo_3traits_full %>% 
  mutate(combo_number = rownames(.)) %>% 
  # count TRUE and FALSE occurrences
  mutate(count = map(
    pairwise_padj_significant_euc,
...
reduced_combinations_full <- bind_rows(
  keep_4trait_full_combination,
  keep_3trait_full_combination
) %>% 
  relocate(number_of_traits, .before = trait_list) %>% 
  relocate(combo_number, .after = number_of_traits) %>% 
  select(number_of_traits, combo_number, trait_list,
...
# 4 traits combo 1: Height, Thickness, Surface area, Height:wet weight
# 3 traits combo 29: Thickness, Surface area, Height:wet weight

# plots together
combo_4traits_full_combo1_biplots <- reduced_combinations_full[[22]][[1]]
combo_3traits_full_combo29_biplots <- reduced_combinations_full[[22]][[74]]

combo_biplots_together <- list(
  combo_4traits_full_combo1_biplots,
  combo_3traits_full_combo29_biplots
)

combo_biplot_file_names <- list(
  "biplot-4traits_full-combo1",
  "biplot_3traits_full-combo29"
)
...
dispersions_full <- reduced_combinations_full %>% 
  select(number_of_traits, combo_number, betadisp_anova) %>% 
  mutate(p_val = map(
    betadisp_anova,
    ~ case_when(

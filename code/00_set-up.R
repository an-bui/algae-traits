...
metadata_sub <- read_sheet(sheet_id, sheet = "00d-metadata-test") 
...
# thallus thickness
thickness <- read_sheet(sheet_id, sheet = "03a-thickness", na = "NA")

# weight <- read_sheet(sheet_id, sheet = "04-weight", na = "NA") 

# wet and dry weight
weight <- read_sheet(sheet_id, sheet = "04a-weight", na = "NA")

# sa_peri <- read_sheet(sheet_id, sheet = "05a-scans") 

# surface area and perimeter
sa_peri <- read_sheet(sheet_id, sheet = "05a-scans_test") 
...
# volume
volume <- read_sheet(sheet_id, sheet = "07a-volume", na = "NA") 
...
irr <- read_csv(here::here("data/SBC-LTER-benthics", "Hourly_Irrandiance_All_Year_20220131.csv")) %>%
  clean_names() %>% 
  # create a sample_ID for each sampling date at each site
  # unite("sample_ID", site, date_local, transect, remove = FALSE) %>% 
  # split up dates into year, monh, and day
  ...
  mutate(daily_sum_umol = sum(light_umol),
         daily_sum_adj_conv = sum(light_adj_conv)) %>% 
  ungroup() %>% 
  # yearly average
  group_by(site, year) %>% 
  mutate(mean_daily_umol = mean(daily_sum_umol, na.omit = TRUE),
         mean_daily_adj_conv = mean(daily_sum_adj_conv, na.omit = TRUE)) %>% 
  ungroup() %>% 
  # take unique values
  select(site, year, mean_daily_umol, mean_daily_adj_conv) %>% 
  unique() %>% 
  # make year a numeric
...
tbspp_matrix <- metadata_ind %>% 
  ...
  select(specimen_ID, sp_code,
         fvfm_mean, thickness_mm_mean, total_dry, ldmc_mean,
...
#### * m. trait by species matrix function ####

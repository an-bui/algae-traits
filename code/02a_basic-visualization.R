...
#                   "ANOVA",
#                   paste0("raw-trait-ANOVA_app1_", today(), ".docx")),
#              pluck(raw_anova_table, 3, 1))
# 
# save_as_docx(path = here("tables",
#                          "ANOVA",
#                          paste0("raw-trait-ANOVA_app2_", today(), ".docx")),
#              pluck(raw_anova_table, 3, 2))
# 
# save_as_docx(path = here("tables",
#                          "ANOVA",
#                          paste0("log-trait-ANOVA_app1_", today(), ".docx")),
#              pluck(log_anova_table, 3, 1))
# 
# save_as_docx(path = here("tables",
#                          "ANOVA",
#                          paste0("log-trait-ANOVA_app2_", today(), ".docx")),
#              pluck(log_anova_table, 3, 2))
# 
# save_as_docx(path = here("tables",
#                   "ANOVA",
#                   paste0("raw-trait-ANOVA_variances_app1_", today(), ".docx")),
#              pluck(raw_variance_tables, 3, 1))
# 
# save_as_docx(path = here("tables",
#                   "ANOVA",
#                   paste0("raw-trait-ANOVA_variances_app2_", today(), ".docx")),
#              pluck(raw_variance_tables, 3, 2))
# 
# save_as_docx(path = here("tables",
#                          "ANOVA",
#                          paste0("log-trait-ANOVA_variances_app1_", today(), ".docx")),
#              pluck(log_variance_tables, 3, 1))
# 
# save_as_docx(path = here("tables",
#                          "ANOVA",
#                          paste0("log-trait-ANOVA_variances_app2_", today(), ".docx")),
#              pluck(log_variance_tables, 3, 2))
# 
# save_as_docx(path = here("tables",
#                          "ANOVA",
#                          paste0("kw-tables_app1_", today(), ".docx")),
#              pluck(kw_tables, 3, 1))
# 
# save_as_docx(path = here("tables",
#                          "ANOVA",
#                          paste0("kw-tables_app2_", today(), ".docx")),
#              pluck(kw_tables, 3, 2))
...
coarse_traits |> 
  ...
  select(growth_form, scientific_name, sp_code, taxon_phylum) |> 
  mutate(growth_form = case_when(
    growth_form == "corticated_macrophytes" ~ "Corticated macrophyte",
    growth_form == "corticated_foliose" ~ "Corticated foliose",
    str_detect(growth_form, "articulated") ~ "Articulated calcareous",
    str_detect(growth_form, "leathery") ~ "Leathery macrophyte"
  )) |> 
  mutate(scientific_name = case_when(
    sp_code == "CC" ~ "Chondracanthus spp. (C. corymbiferus, C. exasperatus)",
    sp_code == "DP" ~ "Dictyota spp. (D. binghamiae, D. flabellata, D. coriacea)",
    TRUE ~ scientific_name
  )) |> 
  arrange(fct_relevel(sp_code, 
                      algae_spcode_factors)) |> 
  flextable() |> 
  merge_v(j = ~ growth_form) |> 
  set_header_labels(taxon_phylum = "Lineage",
  ...
  italic(j = 2, 
         part = "body") |> 
  save_as_docx(path = here("tables", "sp_names.docx"))
